...
class LiveEventHouseView(HouseAccountMixin, EventSecurityMixin, UserPassesTestMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if json_data:
            ...
            # try:
            #     # stream_amount = int(json_data['stream_amount'])
            #     stream_amount = 1
            #     # participants = int(json_data['participants'])
            #     participants = 2
            #     # recording = json_data['recording']
            #     recording = True
            #     # broadcasting = json_data['broadcasting']
            #     broadcasting = True
            ...
            #     # Grab the last EventLiveFee object for the day
            #     # If there isn't one create one 
            #     today = timezone.now().day
            #     try: 
            #         event_live_fee = EventLiveFee.objects.get(event_live=event_live, created_at__day=today)
            #         arqam_house_service_fee = event_live_fee.arqam_house_service_fee

            #         # 1.  ----------------   Calculate session cost -------------------
            #         subscribed_mins = (participants * 3)
            #         if recording:
            #             archived_mins = 3
            #         else:
            #             archived_mins = 0

            #         subscribed_mins_fee = 0.005 * subscribed_mins
            #         archived_mins_fee = 0.04 * archived_mins

            #         session_fee = subscribed_mins_fee + archived_mins_fee
            #         session_fee = decimal.Decimal(session_fee)

            #         print(f"The session fee is {session_fee}")

            #         house_balance = HouseBalance.objects.get(house=event.house)
            #         house_balance.balance = house_balance.balance - session_fee
            #         house_balance.gross_balance = house_balance.gross_balance - session_fee
            #         house_balance.save()

            #         house_balance_log = HouseBalanceLog.objects.get(house_balance=house_balance, arqam_house_service_fee=arqam_house_service_fee)
            #         house_balance_log.balance = house_balance_log.balance - session_fee
            #         house_balance_log.gross_balance = house_balance_log.gross_balance - session_fee
            #         house_balance_log.save()
            #         # ------------------------ Calculate session cost ---------------------

            #         # 2. ---------------------- Calculate total cost -----------------------
            #         arqam_house_service_fee.amount = arqam_house_service_fee.amount + session_fee
            #         arqam_house_service_fee.save()
            ...
            #     except Exception as e:
            #         print(e)
            #         arqam_house_service_fee = ArqamHouseServiceFee.objects.create(house=event.house, live_video=True, amount=decimal.Decimal(0.00))
            #         event_live_fee = EventLiveFee.objects.create(event_live=event_live, arqam_house_service_fee=arqam_house_service_fee)
            #         house_balance = HouseBalance.objects.get(house=event.house)
            #         house_balance.balance = house_balance.balance - decimal.Decimal(0.00)
            #         house_balance.gross_balance = house_balance.balance - decimal.Decimal(0.00)
            #         house_balance.save()
            #         house_balance_log = HouseBalanceLog.objects.create(house_balance=house_balance, balance=house_balance.balance,
            #                                                            gross_balance=house_balance.gross_balance, arqam_house_service_fee=arqam_house_service_fee)
